...
<p align="center">
<a href="#一界面布局">界面布局</a> · <a href="#二对话操作">对话操作</a> · <a href="#三多标签系统">多标签系统</a> · <a href="#四权限控制">权限控制</a> · <a href="#五项目管理">项目管理</a> · <a href="#六模型与提供商">模型与提供商</a> · <a href="#七im-适配器接入">IM 适配器</a> · <a href="#八定时任务">定时任务</a> · <a href="#九远程访问">远程访问</a>
</p>
...
> 允许局域网内的手机、平板或其他电脑通过浏览器访问你的桌面端。

### 启用步骤

1. 打开**设置 → 远程访问**标签页
2. 勾选「启用远程访问」（立即生效）
3. 设置**绑定地址**（默认 `0.0.0.0` 监听所有网卡，也可指定如 `192.168.1.100`）
4. 设置**监听端口**（默认 `8080`）
5. 设置**访问密码**（远程设备登录时需要）
...
你会看到一个登录页面，输入设置的密码即可进入应用。

### 开发模式 vs 生产模式

| 模式 | 命令 | 适用场景 |
|------|------|----------|
| **生产模式** | `SERVER_PORT=3456 bun run src/server/index.ts --remote-enabled --remote-port 8080` | 完整测试 |
| **开发热更新** | `cd desktop && bun run dev:remote` | 前端开发 |

> 生产模式下远程服务器 serve 的是构建产物 `desktop/dist/`，**改前端后需要 `cd desktop && bun run build` 才能生效**。开发模式使用 Vite，热更新，无需 build。

### 安全机制

- 强制密码登录（PBKDF2-SHA256 哈希存储）
- JWT Token 7 天有效期，过期需重新登录
- 本地 Tauri 应用不受影响，无需登录
- 仅限局域网 HTTP，公网使用建议搭配 HTTPS 反向代理
